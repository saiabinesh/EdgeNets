...
def main(args):
    ...
    if args.dataset in ['coco', 'pascal','handsynth']:
        if args.dataset == 'pascal':
            ...
            print("size(val_loader) ",len(dataset_class))
            total_instances_all_type=0
            total_instances_dim_0=0
            for i, (input, boxes,target) in enumerate(dataset_class):
                total_instances_all_type+=len(target)
                # target = target.max(dim=1)[0]
                # num_instances_t=target.sum()
                # num_instances=target.sum(dim=0)
                # total_instances_all_type+=num_instances_t
                # total_instances_dim_0+=num_instances
                # if i==0:
                    # print(type(target))
                    # print("            num_instances_t=target.sum() ",num_instances_t)
                    # print("            num_instances=target.sum(dim=0)",num_instances)
            print("        total_instances_all_type ",total_instances_all_type)
            print("total_instances_dim_0 ",total_instances_dim_0)
            exit()
