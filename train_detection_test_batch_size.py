...
from utilities.train_eval_detect_test_batch_size import train, validate
...
def main(args):
    ...
    # -----------------------------------------------------------------------------
    for batch_size_test in [174,178,182,186,188]:
        print("Batch size tested: ",batch_size_test)
        train_loader = DataLoader(train_dataset, batch_size=batch_size_test, shuffle=True, num_workers=args.workers,
                                  pin_memory=True)
        # val_loader = DataLoader(val_dataset, batch_size=batch_size_test, shuffle=False, num_workers=args.workers,
        #                         pin_memory=True)
        extra_info_ckpt = '{}_{}'.format(args.model, args.s)
        # for epoch in range(start_epoch, args.epochs):
        curr_lr = lr_scheduler.step(0)
        optimizer.param_groups[0]['lr'] = curr_lr

        #print_info_message('Running epoch {} at LR {}'.format(0, curr_lr))
        train_loss, train_cl_loss, train_loc_loss = train(train_loader, model, criterion, optimizer, device, epoch=1)
