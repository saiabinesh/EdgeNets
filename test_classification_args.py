...
def main(args):
    ...
    # create model
    with open(args.logs_file, "r") as in_file:
        raw_lines = in_file.readlines() #sum(line.count("d") for line in in_file)
    # print(reversed(raw_lines[0:100]))
    ...
    if not weights:
        ...
        weights = model_weight_map[weight_file_key]

    num_gpus = torch.cuda.device_count()
    device = 'cuda' if num_gpus >=1 else 'cpu'
    weight_dict = torch.load(weights, map_location=torch.device(device))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logs-file', default='', type=str, help='logs size')
