...
# done_count=1
# not_there=[x for x in classes if not x in os.listdir(source_dir+"/train")]
...
#             # exit()
#             move(full_sub_dir, full_dest_dir)
#             if done_count%100==0:
#                 print(done_count)
#             done_count+=1
# else:
...
# Move the directories for each class from the source directory to the destination directory
start_time = datetime.datetime.now()
...
for split in ["train", "val", "test"]:
    for folder in classes:
        ...
        if os.path.exists(source_sub_dir) and not os.path.exists(dest_sub_dir):
            shutil.move(source_sub_dir, dest_sub_dir)
